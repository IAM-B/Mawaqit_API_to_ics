...
class DocsRequestHandler(SimpleHTTPRequestHandler):
    ...
    def serve_markdown(self, path):
        ...
        try:
            ...
            # Render the template
            rendered_html = self.template.render(title=title, content=html_content)
            ...
            self.send_error(500, f"Error during processing: {e!s}")
...
def run_server(port=8000):
    ...
    print(
        f"   - Documentation Updates: http://localhost:{port}/documentation_updates.md"
    )
