...
from app.modules.slot_utils import (
    adjust_slots_rounding,
    parse_time_str,
    round_down_to_hour,
    round_up_to_hour,
)
...
def test_round_down_to_hour():
    ...
    # Test various times
    assert round_down_to_hour(datetime(2024, 3, 15, 14, 30, 45)) == datetime(
        2024, 3, 15, 14, 0, 0
    )
    assert round_down_to_hour(datetime(2024, 3, 15, 14, 0, 0)) == datetime(
        2024, 3, 15, 14, 0, 0
    )
    assert round_down_to_hour(datetime(2024, 3, 15, 14, 59, 59)) == datetime(
        2024, 3, 15, 14, 0, 0
    )
    assert round_down_to_hour(datetime(2024, 3, 15, 0, 0, 0)) == datetime(
        2024, 3, 15, 0, 0, 0
    )
    assert round_down_to_hour(datetime(2024, 3, 15, 23, 59, 59)) == datetime(
        2024, 3, 15, 23, 0, 0
    )
...
def test_round_up_to_hour():
    ...
    # Test various times
    assert round_up_to_hour(datetime(2024, 3, 15, 14, 30, 45)) == datetime(
        2024, 3, 15, 15, 0, 0
    )
    assert round_up_to_hour(datetime(2024, 3, 15, 14, 0, 0)) == datetime(
        2024, 3, 15, 14, 0, 0
    )
    assert round_up_to_hour(datetime(2024, 3, 15, 14, 59, 59)) == datetime(
        2024, 3, 15, 15, 0, 0
    )
    assert round_up_to_hour(datetime(2024, 3, 15, 0, 0, 0)) == datetime(
        2024, 3, 15, 0, 0, 0
    )
    assert round_up_to_hour(datetime(2024, 3, 15, 23, 59, 59)) == datetime(
        2024, 3, 16, 0, 0, 0
    )
...
def test_adjust_slots_rounding():
    ...
    assert (
        adjusted[0]["start"].hour == 4 and adjusted[0]["start"].minute == 33
    )  # After Fajr
    assert (
        adjusted[2]["end"].hour == 12 and adjusted[2]["end"].minute == 54
    )  # Before Dohr
    assert (
        adjusted[4]["end"].hour == 16 and adjusted[4]["end"].minute == 44
    )  # Before Asr

    # Check intermediate slots (should be rounded)
    assert (
        adjusted[1]["start"].hour == 6 and adjusted[1]["start"].minute == 0
    )  # Rounded
    assert (
        adjusted[3]["start"].hour == 14 and adjusted[3]["start"].minute == 0
    )  # Rounded up
