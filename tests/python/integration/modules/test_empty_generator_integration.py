...
from app.modules.empty_generator import (
    to_datetime,
    format_duration,
    generate_empty_slot_events,
    generate_empty_by_scope
)
...
def test_generate_empty_slot_events(tmp_path, app):
    """Test generating ICS file for empty slots between prayers"""
    with app.app_context():
        ...
        output_file = tmp_path / "test_empty_slots.ics"
        
        # Generate ICS file
        result_path = generate_empty_slot_events(
        ...
        with open(result_path, "rb") as f:
            ...
            assert b"Empty slot" in content
            assert b"Free time slot between prayers" in content

def test_generate_empty_by_scope_today(tmp_path, app):
    """Test generating empty slots ICS file for today"""
    with app.app_context():
        ...
        # Generate ICS file
        output_path = generate_empty_by_scope(
...
def test_generate_empty_by_scope_month(tmp_path, app):
    """Test generating empty slots ICS file for a month"""
    with app.app_context():
...
def test_generate_empty_by_scope_year(tmp_path, app):
    """Test generating empty slots ICS file for a year"""
    with app.app_context():
...
def test_generate_empty_by_scope_invalid_scope(tmp_path, app):
    """Test generating empty slots ICS file with invalid scope"""
    with app.app_context():
