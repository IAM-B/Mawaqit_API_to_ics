...
class TestPlannerAjaxEndpoint:
    ...
    def test_planner_ajax_success_today(self, client):
        ...
        with patch(
            ...
            return_value=(mock_prayer_times, "Europe/Paris"),
        ):
            with patch(
                "app.views.planner_view.generate_prayer_ics_file",
                return_value="prayer.ics",
            ):
                with patch(
                    "app.views.planner_view.generate_empty_by_scope",
                    return_value="empty.ics",
                ):
                    with patch(
                        "app.views.planner_view.generate_slots_by_scope",
                        return_value="slots.ics",
                    ):
                        with patch(
                            "app.views.planner_view.segment_available_time",
                            return_value=[],
                        ):
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch(
                                "app.views.planner_view.get_mosque_info_from_json",
                                return_value=None,
                            ):
                                with patch(
    ...
    def test_planner_ajax_fallback_to_web(self, client):
        ...
        with patch(
            ...
            with patch(
                "app.views.planner_view.get_mosque_info_from_json", return_value=None
            ):
                with patch(
                    "app.views.planner_view.fetch_mawaqit_data",
                    return_value=mock_web_data,
                ):
                    with patch(
                        "app.views.planner_view.generate_prayer_ics_file",
                        return_value="prayer.ics",
...
class TestPlannerViewErrorHandling:
    ...
    def test_handle_planner_ajax_invalid_padding_type(self, client):
        ...
        assert (
            "invalid literal for int()" in data["error"]
            or "Invalid padding values" in data["error"]
        )
    ...
    def test_handle_planner_ajax_invalid_coordinates(self, client):
        ...
        with patch(
            ...
            assert (
                ...
                or "Invalid coordinates" in data["error"]
            )
