...
from app.modules.mawaqit_fetcher import fetch_mawaqit_data, fetch_mosques_data
...
from app.modules.time_segmenter import (
    generate_empty_slots_for_timeline,
    segment_available_time,
)
    ...
    if normalized_data:
        ...
        for day_num in range(1, days_in_month + 1):
            if day_num > len(normalized_data):
                ...
                print(
                    f"📅 Generated data for day {day_num}/{current_month} using template"
                )
    ...
    for month_index, month_days in enumerate(prayer_times, start=1):
        ...
        try:
            if month_index == 12:
                ...
                last_day = datetime(current_year, month_index + 1, 1) - timedelta(
                    days=1
                )
            days_in_month = last_day.day
        except Exception as e:
            ...
            try:
                ...
                if isinstance(time_list, list) and len(time_list) == 6:
                    if all(isinstance(t, str) and ":" in t for t in time_list):
                        ...
                        print(
                            f"⚠️ Invalid time format for {day_str}/{month_index}: {time_list}"
                        )
                else:
                    print(
                        f"⚠️ Invalid time format for {day_str}/{month_index}: {time_list}"
        ...
        if normalized_month:
            ...
            for day_num in range(1, days_in_month + 1):
                ...
                if day_str not in normalized_month:
                    ...
                    print(
                        f"📅 Generated data for day {day_str}/{month_index} using template"
                    )
...
def handle_planner_post(masjid_id, scope, padding_before, padding_after):
    ...
    try:
        ...
        features_options = {
            "include_voluntary_fasts": request.form.get("include_voluntary_fasts")
            == "on",
        ...
        has_individual_paddings = any(
            request.form.get(f"{prayer}_padding_before")
            and request.form.get(f"{prayer}_padding_after")
        ...
        return {
            "error": "Invalid padding values: padding_before and padding_after must be integers"
        }, 400

    if padding_before < 0 or padding_after < 0:
        raise ValueError(
            "Invalid padding values: padding_before and padding_after must be positive integers"
        )
    ...
    try:
        ...
        if scope == "month":
            prayer_times = normalize_month_data(
                prayer_times, include_sunset=include_sunset
            )
        elif scope == "year":
            prayer_times = normalize_year_data(
                prayer_times, include_sunset=include_sunset
            )
        ...
        if scope == "today":
            if isinstance(prayer_times, dict):
                slots = segment_available_time(
                    prayer_times, tz_str, padding_before, padding_after, prayer_paddings
                )
            ...
            if scope == "month":
                ...
                for i, daily in enumerate(prayer_times):
                    if not isinstance(daily, dict):
                        print(
                            f"⚠️ Unexpected format for day {i + 1}: {type(daily)} → {daily}"
                        )
                        continue
                    try:
                ...
                for month_index, month_days in enumerate(prayer_times, start=1):
                    ...
                    for day_str, times_dict in month_days.items():
                        try:
                            ...
                            print(
                                f"⚠️ Error processing day {day_str} in month {month_index}: {e}"
                            )
                    segments.append(
                        {
                            "month": month_index,
...
def handle_generate_ics():
    ...
    try:
        ...
        if scope not in ("today", "month"):
            return {
                "error": "Invalid scope. Only 'today' and 'month' are supported for ICS generation"
            }, 400

        print(
            f"📥 ICS Generation Request: masjid_id={masjid_id}, scope={scope}, padding_before={padding_before}, padding_after={padding_after}"
        )
        ...
        # Return JSON response
        return {"success": True, "ics_path": ics_path, "scope": scope}
